...
(() => {
  ...
  // Store at up to 3000px on the longest side so the lightbox can display
  // images at full-screen resolution without pixelation. WebP at q=0.88
  // keeps file sizes reasonable even at this ceiling.
  const MAX_DIM = 3000;
  ...
  async function toDataUrl(file, targetW) {
    ...
    try {
      // Always cap at MAX_DIM regardless of slot render size — the lightbox
      // displays images at full-screen, not at the thumbnail cell's width.
      const cap = MAX_DIM;
  ...
  const stylesheet =
    ...
    // Lazy-load fade so rehydrated images don't pop in.
    '.frame img{opacity:0;transition:opacity .4s ease}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      // Also flag loaded for the opacity fade-in.
    ...
    connectedCallback() {
      ...
      // View-mode guard: when window.__GALLERY_EDIT is falsy (default),
      // skip drag/drop + click-to-browse so the slot is purely a display.
      this._editMode = !!window.__GALLERY_EDIT;
      if (this._editMode) {
    ...
    _render() {
      ...
      // View-mode guard: hide all empty-state chrome when not editing.
      // Re-read every render so toggling window.__GALLERY_EDIT takes effect.
      const editMode = !!window.__GALLERY_EDIT;
      if (!editMode) {
        this._empty.style.cursor = 'default';
        this._empty.style.pointerEvents = 'none';
      } else {
        this._empty.style.cursor = '';
        this._empty.style.pointerEvents = '';
      }
      // Toggle via style.display — the [hidden] attribute alone loses to
      // the display:flex / display:block rules in the stylesheet above.
      if (url) {
        if (this._img.getAttribute('src') !== url) {
          // New src — reset the fade so the swap also gets the transition.
        ...
        // In view mode, hide the empty UI entirely — just show the host's
        // background colour (the .gallery-item #D8D3C4 tile).
        this._empty.style.display = editMode ? 'flex' : 'none';
